...
Vagrant.configure("2") do |config|
  ...
  [1, 2, 3].each do |i|
    config.vm.define "server#{i}" do |web|
      web.vm.network "private_network", ip: "192.168.121.21#{i}"
  ...
  - hosts: imageservers
    ...
    roles:
      ...
      - image
EOF
  	PYTHONUNBUFFERED=1 ANSIBLE_FORCE_COLOR=true ansible-playbook -i local app.yml
        )
        rm -rf ansible-isucon
      SHELL
      ...
      if i != 3 then
        web.vm.provision "shell", inline: <<-SHELL
          sed -ie 's/localhost/192.168.121.213/' /home/isucon/env.sh
        ...
        web.vm.provision "shell", inline: <<-SHELL
          ...
          systemctl disable isubata.golang
        SHELL
      end      
    end
  end
  ...
  config.vm.define "bench" do |bench|
    bench.vm.network "private_network", ip: "192.168.121.214"
